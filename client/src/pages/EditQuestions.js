...
// 2. axios patch url
// 3. axios patch body
// 4. axios navigate url
function EditQuestions() {
  const location = useLocation();
  const { title, body, tags, questionId } = location.state;
  ...
  const [newTagList, setNewTagList] = useState(tags);
  ...
  const deleteTagItem = (e, index) => {
    e.preventDefault();
    const deleteTagItem = newTagList[index];
    const filteredTagList = newTagList.filter(
      (tagItem) => tagItem !== deleteTagItem
    );
    setNewTagList(filteredTagList);
  };
      ...
      <S_Content>
        <form onSubmit={handleSubmit}>
          <S_Main>
            ...
            <S_FormSubmit>
              <S_BtnSubmit type="submit">Edit your question</S_BtnSubmit>
            </S_FormSubmit>
          </S_Main>
        </form>
      </S_Content>
